...
class BatchOperationsController
  ...
  Future<BatchOperationResult> cancelAppointments(
      ...
      await Future.wait(
        appointments.map((appointment) async {
          try {
            ...
            failureCount++;
            errors.add(
              'Erro ao cancelar agendamento de ${appointment.clientName}: $e',
            );
          }
        }),
      );
  ...
  Future<BatchOperationResult> confirmAppointments(
    List<Appointment> appointments,
  ) async {
    state = const AsyncValue.loading();
      ...
      await Future.wait(
        appointments.map((appointment) async {
          try {
            ...
            failureCount++;
            errors.add(
              'Erro ao confirmar agendamento de ${appointment.clientName}: $e',
            );
          }
        }),
      );
...
final batchOperationsControllerProvider =
    ...
    >((ref) {
      ...
      return BatchOperationsController(
        appointmentsService,
        notificationService,
        ref.read,
      );
